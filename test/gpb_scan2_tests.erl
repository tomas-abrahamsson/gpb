...
line_numbers_in_tokens_test() ->
    ...
    {ok, [{{str_lit,_}, 1},
        ...
        gpb_scan2:binary(B),
    {ok, Tokens, 9} = gpb_scan2:binary(<<B/binary, $\n>>),
    ok.
...
ok_scan(B) when is_binary(B) ->
    {ok, TokensLines, _EndLine} = gpb_scan2:binary(B),
    [Token || {Token, _Line} <- TokensLines].

err_scan(B, ExpectedErrTextFragmentsLowerCase) when is_binary(B) ->
    {error, {Line, Mod, Reason}, _EndLine} = gpb_scan2:binary(B),

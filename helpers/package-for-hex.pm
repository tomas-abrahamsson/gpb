...
set -e
./helpers/mk-versioned-archive --dest-dir="$d"
cd "$d"
tar xfpz gpb-"$vsn".tar.gz
cd gpb-"$vsn"
...
defmodule Gpb.Mixfile do
  ...
  defp package do
    [files: ~w(src include descr_src priv bin build gpb.vsn COPYING.LIB README.* rebar.config.script ChangeLog Makefile),

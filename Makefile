...
.PHONY: all clean test doc xref dialyze build_plt clean_plt proper
...
PROP_MODULES := \
	$(patsubst $(test)/%.erl,%,$(wildcard $(test)/prop_*.erl))
...
PROP_BEAMS   := $(patsubst %,$(test)/%.beam,$(PROP_MODULES))
...
proper-test: all $(PROP_BEAMS) proper FORCE
	@echo PropEr testing...
	$(silencer)$(ERL) $(ERL_BATCH_FLAGS) -pa $(test) -pa $(ebin) \
	    -pa .deps/proper/ebin -eval " \
            Props = [{M,F} \
                     || M <- [$(subst $(space),$(comma),$(PROP_MODULES))], \
                     {F,0} <- M:module_info(exports), \
                     \"prop_\"++_ <- [atom_to_list(F)]], \
            Res = lists:foldl( \
                    fun({M,F}, Acc) -> \
                            io:format(\"Testing property ~p:~p()~n\", [M,F]), \
                            case proper:quickcheck(M:F()) of \
                                true -> Acc; \
                                _Err -> error \
                            end \
                    end, \
...
$(test)/prop_%.beam: $(test)/prop_%.erl | $(ebin) proper
	$(ERLC) $(ERLC_FLAGS) $(EUNIT_ERLC_FLAGS) -pa $(ebin) \
		-pa .deps/proper/ebin -o $(test) $<
